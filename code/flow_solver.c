...
const char *SEARCH_RESULT_STRINGS[4] = {"successful", "unsolvable",
                                        "out of memory", "in progress"};
...
// Mock time for WASM (deterministic is fine, or simple counter)
...
// Terminal color check removed for WASM
int terminal_has_color() { return 0; }
...
const char *color_char(const char *ansi_code, char color_out, char mono_out) {
  ...
  if (g_options.display_color) {
    snprintf(buf, 256, "\033[30;%sm%c\033[0m", ansi_code, color_out);
  } else {
...
const char *unprint_board(const game_info_t *info) { return ""; }
...
uint8_t cell_get_type(cell_t c) { return c & 0x3; }

//////////////////////////////////////////////////////////////////////
// Get the direction from a cell value

uint8_t cell_get_direction(cell_t c) { return (c >> 2) & 0x3; }

//////////////////////////////////////////////////////////////////////
// Get the color from a cell value

uint8_t cell_get_color(cell_t c) { return (c >> 4) & 0xf; }
...
const char *color_cell_str(const game_info_t *info, cell_t cell) {
  ...
  case TYPE_FREE:
    ...
    break;
  case TYPE_PATH:
    return color_char(l->ansi_code, DIR_CHARS[dir], l->display_char);
    break;
  default:
    return color_char(l->ansi_code, (type == TYPE_INIT ? 'o' : 'O'),
                      l->display_char);
  }
}
...
// SVG printing removed
void game_print_svg(FILE *fp, const game_info_t *info,
...
int game_num_free_coords(const game_info_t *info, const game_state_t *state,
  ...
  for (int dir = 0; dir < 4; ++dir) {
    ...
    if (neighbor_pos != INVALID_POS && state->cells[neighbor_pos] == 0) {
      ++num_free;
    }
  }
...
double game_make_move(const game_info_t *info, game_state_t *state, int color,
                      int dir, int forced) {
  ...
  if (goal_dir >= 0) {
    ...
    state->cells[info->goal_pos[color]] =
        cell_create(TYPE_GOAL, color, goal_dir);
    state->completed |= (1 << color);
    action_cost = 0;
...
// Helper function for below (removed detect_format)

// (Removed orphan code)
//////////////////////////////////////////////////////////////////////
// Read game board from string buffer
int game_read_buffer(const char *input_buffer, game_info_t *info,
                     game_state_t *state) {
  ...
  for (size_t color = 0; color < info->num_colors; ++color) {
    ...
    // Always do outside-in optimization check
...
// Hint reading removed
int game_read_hint(const game_info_t *info, const game_state_t *state,
                   const char *filename, uint8_t hint[MAX_CELLS]) {
  return 0;
}
  ...
  if (!info->user_order && g_options.order_most_constrained) {
    ...
    */
    ...
    for (size_t i = 0; i < info->num_colors; ++i) {
      ...
      int num_free = game_num_free_pos(info, state, state->pos[color]);
...
pos_t region_find(region_t *regions, pos_t p) {
  if (regions[p].parent != INVALID_POS && regions[p].parent != p) {
...
void region_unite(region_t *regions, pos_t a, pos_t b) {
...
int game_is_deadend(const game_info_t *info, const game_state_t *state,
                    pos_t pos) {
...
int game_regions_stranded(const game_info_t *info, const game_state_t *state,
  ...
  for (int color = 0; color < info->num_colors; ++color) {
    ...
    game_regions_add_color(info, state, rmap, state->pos[color], cflag,
                           cur_rflags);

    // Add color flag to all regions for goal_pos
    game_regions_add_color(info, state, rmap, info->goal_pos[color], cflag,
                           goal_rflags);
...
int game_is_forced(const game_info_t *info, const game_state_t *state,
                   int color, pos_t pos) {
  ...
  for (int dir = 0; dir < 4; ++dir) {
    pos_t neighbor_pos = pos_offset_pos(info, pos, dir);
    if (neighbor_pos == INVALID_POS || neighbor_pos == state->pos[color]) {
      continue;
      ...
      for (size_t other_color = 0; other_color < info->num_colors;
        ...
        if (state->completed & (1 << other_color)) {
          continue;
        }
...
int game_find_forced(const game_info_t *info, const game_state_t *state,
                     int *forced_color, int *forced_dir) {
...
void node_storage_unalloc(node_storage_t *storage, const tree_node_t *n) {
...
void node_storage_destroy(node_storage_t *storage) { free(storage->start); }
...
int heapq_empty(const queue_t *q) { return q->heapq.count == 0; }
...
void heapq_destroy(queue_t *q) { free(q->heapq.start); }
...
int fifo_empty(const queue_t *q) { return q->fifo.next == q->fifo.count; }
...
void fifo_destroy(queue_t *q) { free(q->fifo.start); }
...
tree_node_t *node_create(node_storage_t *storage, tree_node_t *parent,
                         const game_info_t *info, const game_state_t *state) {
...
void node_update_costs(const game_info_t *info, tree_node_t *n,
                       size_t action_cost) {
...
void game_animate_solution(const game_info_t *info, const tree_node_t *node) {
...
int game_is_free(const game_info_t *info, const game_state_t *state, int x,
  ...
  return (coords_valid(info, x, y) && state->cells[pos_from_coords(x, y)] == 0);
}
...
int game_check_chokepoint(const game_info_t *info, const game_state_t *state,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    assert( state_copy.cells[pos_offset_pos(info, state_copy.pos[color], dir)]
    == 0 );
    */
  ...
  int result =
      game_regions_stranded(info, &state_copy, rcount, rmap, color, n + 1);
...
void game_diagnostics(const game_info_t *info, const tree_node_t *node) {
...
tree_node_t *game_validate_ff(const game_info_t *info, tree_node_t *node,
                              node_storage_t *storage) {
  ...
  if (g_options.search_fast_forward && g_options.order_forced_first) {
    ...
    if (game_find_forced(info, node_state, &color, &dir)) {

      if (!game_can_move(info, node_state, color, dir)) {
        goto unalloc_return_0;
      }

      tree_node_t *forced_child = node_create(storage, node, info, node_state);
...
int game_search(const game_info_t *info, const game_state_t *init_state,
                const uint8_t *hint, double *elapsed_out, size_t *nodes_out,
                game_state_t *final_state) {

  size_t max_nodes = g_options.search_max_nodes;

  if (!max_nodes) {
    max_nodes = floor(g_options.search_max_mb * MEGABYTE / sizeof(tree_node_t));
  }
  ...
  if (!g_options.display_quiet) {
    ...
    printf("will search up to %'zu nodes (%'.2f MB)\n", max_nodes,
           max_nodes * (double)sizeof(tree_node_t) / MEGABYTE);
  ...
  while (result == SEARCH_IN_PROGRESS) {
    ...
    for (int dir = 0; dir < 4; ++dir) {
      ...
      if (game_can_move(info, &n->state, color, dir)) {
        ...
        size_t action_cost =
            game_make_move(info, &child->state, color, dir, forced);
  ...
  if (!g_options.display_quiet) {
    ...
    double storage_mb =
        (storage.count * (double)sizeof(tree_node_t) / MEGABYTE);

    printf("\nsearch %s after %'.3f seconds and %'zu nodes (%'.2f MB)\n",
           SEARCH_RESULT_STRINGS[result], elapsed, storage.count, storage_mb);
    ...
    if (result == SEARCH_SUCCESS) {
      ...
      printf("final cost to come=%'g, cost to go=%'g\n",
             solution_node->cost_to_come, solution_node->cost_to_go);
...
const char *solve_puzzle_wasm(const char *input_str) {
  ...
  if (result == SEARCH_SUCCESS) {
    ...
    // We want raw ASCII codes for the colors, 0 for empty.
    ...
    for (size_t y = 0; y < info.size; ++y) {
      ...
      for (size_t x = 0; x < info.size; ++x) {
        ...
        int output_code = 0;
        if (type != TYPE_FREE) {
          // Map color index back to original char if possible,
          // but here we just return the "standard" char for that color index
          // based on get_color_id reverse lookup or just send the ASCII of the
          // standard set The JS side expects ASCII codes.

          // Color ID -> Char
          // We need to look up in the static color_dict using the
          // info.color_ids
          output_code = color_dict[info.color_ids[color]].input_char;
        }
